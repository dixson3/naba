...
naba is a single self-contained binary — no runtime, no dependencies. The **vendor
...
`~/.config/naba/naba-receipt.json`. This is the recommended way in: unlike Homebrew, a vendor
install can update itself in place with [`naba self update`](#self-update):

```bash
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/dixson3/naba/releases/latest/download/naba-installer.sh | sh
```

> Available once the first tagged release is published — until then, install via
> [Homebrew](#homebrew-macos-and-linux) or [from source](#build-from-source).

### Bootstrap (`naba.ysapp.net`)

A shorter, memorable form of the vendor install above. The site hosts `install.sh` — a mirror
of the cargo-dist installer — so the first install is just:
...
If you already live in Homebrew, the tap works too — but a Homebrew install does **not**
