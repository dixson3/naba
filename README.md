...
`naba` is a single self-contained binary — no runtime, no dependencies.


### Vendor Install (from `naba.ysapp.net`)

The site hosts `install.sh` — so the first install is just:

```bash
curl --proto '=https' --tlsv1.2 -LsSf https://naba.ysapp.net/install.sh | sh
```

GitHub Releases remains canonical for every binary.


### Homebrew (macOS and Linux)

If you already live in Homebrew, the you can use the tap — but a Homebrew install does **not**
self-update via `naba self update`; upgrades come through `brew upgrade`:
...
A successful `self update` also refreshes the installed harness skills (`naba skills
upgrade`) unless you pass `--binary-only`. `version` and `doctor` print a throttled, offline
"update available" nudge for vendor installs.

You can set `NABA_NO_UPDATE_CHECK=1` (or run in `CI`) to silence it.
...
`providers.<provider>.api-key` takes precedence over the env var named by `providers.<provider>.api-key-envvar` which in turn takes precedence over
...
`naba` prefers the bearer token when one resolves, else signs with
SigV4. The region defaults to `us-east-1`, resolved in this order (highest precedence first):
...
`--provider` is a usage error (exit 2).
...
For Gemini, the model is resolved in this order (highest precedence first):
...
Existing configs (e.g. `model: gemini-2.5-flash-image`) keep working unchanged.
...
up-to-date, complete, unmodified); the **effective provider's** API key present (it resolves the
effective provider — CLI `--provider`, else config `provider`, else env-key autodetect — then
checks that provider's key); the
