...
name: naba-brand-kit
description: >
  Generate a coordinated brand asset set (icon + pattern + hero image) via the naba
  CLI. TRIGGER when: /naba-brand-kit invoked, or the user wants a cohesive set of brand
  visuals in one pass. SKIP for: a single asset — use /naba-icon, /naba-pattern, or /naba-generate
  directly.
user-invocable: true
skill-group: naba
depends-on-tool: [naba]
depends-on-skill: [naba-icon, naba-pattern, naba-generate]
...
1. **Gather brand details**: Ask the user for:
...
5. **Present all assets**: Show all output file paths grouped by type. Use the Read tool to display each asset.

6. **Offer iteration**: Ask which assets need refinement and re-run individual commands as needed.
