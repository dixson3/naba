# plan-008-james-dixson-24173a

> Rename naba CLI --surface to --harness; install skills idiomatically per harness (claude-code, opencode, pi, codex) for user+project scope, multi-harness with upgrade-all; author a per-harness skills-layout SPEC that naba validates against; consolidate SPEC.md into docs/specifications; make skills dual-purpose for CLI + MCP (templated render at install/upgrade or MCP reference); integrate docs/diary appropriately
