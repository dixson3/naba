# plan-004-james-dixson-9a7b16

> Add OpenRouter as a model provider alongside Gemini: config gains default provider + default model, provider selectable on CLI like model (provider required when model is specified), evaluate a general Rust multi-provider aggregation library vs a bespoke OpenRouter handler + provider selector
